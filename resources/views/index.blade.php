            ...
            <div id="order-items" class="mb-4">
                @foreach($products as $index => $product)
                    <div class="mb-4">
                        ...
                        <select name="items[{{ $index }}][product_id]" id="product_{{ $index }}" class="border rounded p-2 w-full">
                            <option value="{{ $product->id }}">{{ $product->name }}</option>
                        </select>

                        <label for="variant_{{ $index }}" class="block text-sm font-medium mt-2">Variant</label>
                        @if($product->category_id == 1) <!-- Minuman -->
                            <select name="items[{{ $index }}][variant]" id="variant_{{ $index }}" class="border rounded p-2 w-full">
                        ...
                        @elseif($product->category_id == 3) <!-- Promo -->
                            <select name="items[{{ $index }}][variant]" id="variant_{{ $index }}" class="border rounded p-2 w-full">
                                <option value="default">Default</option>
                            </select>
                        @endif

                        <label for="quantity_{{ $index }}" class="block text-sm font-medium mt-2">Quantity</label>
                        <input type="number" name="items[{{ $index }}][quantity]" id="quantity_{{ $index }}" class="border rounded p-2 w-full" value="1" min="1">
                    </div>
                @endforeach
            </div>
